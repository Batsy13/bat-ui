...
export const NavSidebar = () => {
  return (
    <aside className="pt-12 pl-3">
      <div className="text-[#a7a7a7] px-3">Components</div>
      <div className="py-4 text-white text-[14px] flex items-start gap-1 flex-col">
        {components.map((item) => (
          <a className="bg-transparent hover:bg-[#141414] px-3 py-[5px] cursor-pointer rounded-[5px]" href={`/docs/components${item.path}`}>{item.title}</a>
        ))}
      </div>
    </aside>
  );
